...
# parent_dir = os.path.dirname(curr_dir) + '/ailive' #Use this if error: src not found. Also move the '/streamlit_app/data' folder to the 'ailive' folder, outside the 'src' folder.

sys.path.append(parent_dir + "/src")

from ailive.agent.chat_agent import ChatAgent
...
if selected_llm_model and selected_vision_model:
    ...
    st.write(" ")
    clear_MemCore = st.button("Clear MemCore DB")
    ...
    if clear_MemCore:
        # print("Front end recieved request to clear MemCore")
        chat_agent.clearMemCore()
        st.write("MemCore DB cleared")
    ...
    if generate_clicked:
        ...
        if len(chat_agent.MemCore_stream) > 0:
            # MemCore Stream
            MemCore_items = chat_agent.MemCore_stream.get_MemCore()
            MemCore_items_dicts = [item.to_dict() for item in MemCore_items]
            df = pd.DataFrame(MemCore_items_dicts)
            st.write("MemCore Stream")
            st.dataframe(df)

            # Entity Knowledge Store
            knowledge_MemCore_items = chat_agent.entity_knowledge_store.get_MemCore()
            knowledge_MemCore_items_dicts = [
                item.to_dict() for item in knowledge_MemCore_items
            ]
            df_knowledge = pd.DataFrame(knowledge_MemCore_items_dicts)
